...
| `raw_encoder`           | `str`          | `null` (resolves to `"RLE"` when `data_type = "RAW"`) | How RAW point data is converted to intervals. `"RLE"` run-length encodes: consecutive samples with the same value collapse into a single `[tstart, tend)` interval per run. `"INTERVAL"` only derives `tend` from the next sample's timestamp and drops exact duplicate points, keeping equal-valued runs as separate intervals. Only takes effect with `data_type = "RAW"`; ignored for `"RLE"` input. |
